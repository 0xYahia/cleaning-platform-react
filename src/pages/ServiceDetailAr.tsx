    ...
    <>
      <section className="relative min-h-[600px] flex items-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-[#0F6E56]/70 to-transparent" />
        </div>
        <div className="relative z-10 max-w-7xl mx-auto px-6 w-full">
          <div className="max-w-2xl bg-white/10 backdrop-blur-md p-10 rounded-3xl border border-white/20">
            <span className="inline-block px-4 py-1 bg-secondary-container text-on-secondary-container rounded-full text-label-caps font-label-caps mb-6">
              {service.badgeAr ?? 'خدمة متميزة'}
            </span>
            <h1 className="text-display-lg font-display-lg text-white mb-4">
              {service.titleAr}
            </h1>
            ...
            <div className="flex items-baseline gap-4 mb-8">
              <span className="text-white/80 text-heading-sm">تبدأ من</span>
              <span className="text-white text-display-md font-bold">
                {service.startingPrice} ريال
              </span>
            </div>
            <Link
              to="/ar/booking"
              className="inline-block bg-[#0F6E56] text-white px-10 py-4 rounded-xl text-heading-sm font-bold shadow-xl active:scale-95 transition-all hover:bg-[#0c5a46]"
            >
              احجز الآن
            </Link>
          </div>
      ...
      <section className="py-xl max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-xl">
          <div>
            <h2 className="text-display-md font-display-md mb-md">
              ماذا تشمل الخدمة؟
            </h2>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              {galleryImages.slice(1).map((img, i) => (
                <div key={i} className="overflow-hidden rounded-3xl h-48">
                  <img
                    className="w-full h-full object-cover transition-transform duration-500 hover:scale-105"
                    src={img}
                    alt=""
